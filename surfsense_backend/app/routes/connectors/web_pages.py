"""Web pages connector routes and indexing functions."""
...
router = APIRouter(prefix="/web-pages", tags=["connectors:web-pages"])
...
async def run_web_page_indexing(
    ...
    try:
        ...
        if documents_processed > 0:
            ...
            logger.info(
                f"Web page indexing completed: {documents_processed} pages processed"
            )
        else:
            logger.error(
                f"Web page indexing failed or no pages processed: {error_or_warning}"
            )
    except Exception as e:
        logger.error(f"Error in web page indexing task: {e!s}")
