"""Jira connector routes and indexing functions."""
...
async def run_jira_indexing(
    ...
    try:
        documents_processed, error_or_warning = await index_jira_issues(
            session=session,
        ...
        if documents_processed > 0:
            ...
            logger.info(
                f"Jira indexing completed: {documents_processed} documents processed"
            )
        else:
            logger.error(
                f"Jira indexing failed or no documents processed: {error_or_warning}"
            )
    except Exception as e:
        logger.error(f"Error in background Jira indexing task: {e!s}")
